...
def read_signal(filename):
    ...
    return channel1 + 1j*channel2, sample_rate
...
def segment_signal(ridge_smooth, t_spec, plot=False):
...
def main():
    ...
    [0.7, 0.1, 0.1, 0.1], 
    [0.6, 0.1, 0.1, 0.2]]
    if True: #read file then predit
        with open("received_path.pki", 'rb') as pkl_file:
            loaded_path = pickle.load(pkl_file)
        
        signal, fs = read_signal('received_test.dat')
        Sxx_mag, f, t_spec, ridge_smooth = compute_stft(signal, fs)
        segments = segment_signal(ridge_smooth, t_spec)
        results = classify_segments(segments, ridge_smooth, t_spec, states)
        plot_results(Sxx_mag, f, t_spec, results)
        accuracy = compare_with_path(results, loaded_path)
        ...
        accuracy = 1 - error
        print(f"Run {1}, reconstruct accuracy : {accuracy:.2f}")
